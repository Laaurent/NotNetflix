               ...
               <DialogTitle>
                  <div class="header">
                     ...
                     <div
                        ...
                        :style="show.image ? 'background-image: url(\'' + show.image.original + '\')' : ''"
                     ></div>
                  </div>
               </DialogTitle>
...
export default {
   ...
   setup(props, { emit }) {
      ...
      function submitComment() {
         error.value = "";
         axios
            .post("http://localhost:4000/comments", { content: contentComment.value, showId: show.value.id }, { withCredentials: true })
            .then((res) => {
            ...
            .catch((err) => {
               console.error(err);
               error.value = "Vous devez être connecté pour faire cela.";
            });
      }

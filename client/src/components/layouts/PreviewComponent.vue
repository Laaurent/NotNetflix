  ...
  >
    ...
    >
      <div class="flex items-center justify-center min-h-screen">
        ...
        <div
          class="relative mx-auto bg-neutral-900 pb-6 rounded text-white w-mysize rounded-xl overflow-hidden my-10"
        >
          <DialogTitle>
            <div class="header">
              <div
                class="px-12 header__text z-10 h-mysize w-full flex absolute flex-col justify-between py-8"
              >
                ...
                <div class="header__text_content flex flex-col gap-2 py-4">
                  ...
                  <div
                    class="header__text_content_buttons flex gap-2 items-center"
                  >
                    <button
                      class="btn flex items-center gap-2 px-5 py-1 text-xl rounded bg-white text-black"
                      @click="goToUrl()"
                    >
                      <IconsComponent
                        icon="play"
              ...
              <div
                class="header__gradient bg-gradient-to-t w-full absolute h-5/6 h-mysize w-full from-neutral-900"
              ></div>
              <div
                class="header__bg h-mysize bg-center bg-cover"
                :style="
                  'background-image: url(\'' + show.image.original + '\')'
                "
              ></div>
            </div>
          </DialogTitle>
          ...
          <div class="px-12 flex flex-col gap-8">
              ...
              <div class="description flex gap-10">
                <div class="description__left flex-auto flex flex-col gap-6">
                  ...
                  <div class="description__left_resume">
                    <p
                      class="text-sm"
                      v-html="show.summary.slice(3).slice(0, -4)"
                    ></p>
                  </div>
                </div>
                <div
                  class="description__right flex-none w-60 flex flex-col gap-4 text-sm"
                >
                  <p>
            ...
            <div>
              <div class="episode-header flex justify-between py-4 relative">
                ...
                <Menu>
                  ...
                  ></MenuButton>
                  <MenuItems
                    class="my-1 flex flex-col absolute bg-neutral-800 border-2 border-neutral-700 py-4 right-0 top-16"
                  >
                    <MenuItem
                      class="px-6 py-1"
                      v-slot="{ active }"
                      v-for="i in show_episodes.data[
                        show_episodes.data.length - 1
                      ].season"
                      :key="i"
                    >
                      <button
            ...
            <div
              class="description__right flex-none w-full flex flex-col gap-1 text-sm"
            >
              <p v-if="show.network">
              ...
              <p class="break-normal">
                <span class="text-neutral-500">Distributions :</span> {{ cast }}
              </p>
              <p>
                <span class="text-neutral-500">Genres :</span>
                {{ show.genres.join(", ") }}
...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function getShowCast(id = 0) {
      try {
        show_cast.value = await axios.get(
    ...
    function goToUrl() {
      window.open(
        `https://www.youtube.com/results?search_query=${show.value.name}+season+${season.value}`,
        "_blank"
      );
    }

<template>
   <nav :class="windowTop > 0 ? 'bg-neutral-900' : ' bg-gradient-to-b from-neutral-900'" class="flex w-full items-center fixed px-8 py-4 z-20">
      ...
      <ul class="flex flex-none flex-row items-center">
         <li>
            <label @click="displaySearchbar = true" class="flex cursor-pointer flex-row items-center">
         ...
         <li>
            <Popover class="relative">
               <PopoverButton class="flex flex-row items-center">
                  ...
                  ></div>
                  <ChevronDownIcon class="ml-2 h-5 w-5 text-white transition duration-150 ease-in-out group-hover:text-opacity-90" />
               </PopoverButton>

               <PopoverPanel class="w-[200px] absolute left-1/2 z-10 -translate-x-1/2 transform bg-neutral-900 px-2 py-2 text-sm text-white opacity-90">
                  <ul class="flex flex-auto flex-col">
                     ...
                     <li class="py-4"><hr /></li>
                     <li class="hover:opacity:100 px-4 py-2 hover:bg-slate-700">
...
export default {
   ...
   setup() {
      ...
      const onScroll = () => {
         windowTop.value = window.top.scrollY; /* or: e.target.documentElement.scrollTop */
      };

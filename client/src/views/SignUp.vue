...
export default {
   ...
   setup() {
      ...
      function submitForm() {
         ...
         error.value = null;
         axios
            .post("http://localhost:4000/signup", { ...form })
            .then((res) => {
               console.log(res);
               success.value = "Vous pouvez maintenant vous connecter.";
            })
            .catch((err) => {
               console.log(err);
      ...
      return { error, success, form, submitForm };
   },
};
</script>
   ...
   <div class="bg-neutral-900 text-sm text-white">
      <div class="bg-[url('/img/bg-large-login.jpg')] h-screen flex items-center justify-center">
         ...
         <div class="absolute bg-opacity w-[500px] py-12 px-24 rounded">
            <h1 class="text-3xl pb-8 font-bold">S'inscrire</h1>
            <form class="flex flex-col gap-y-4 items-center justify-center" @submit.prevent="submitForm">
               ...
               <p class="text-green-800 italic text-sm" v-if="success">{{ success }}</p>
            </form>
         </div>
      </div>
